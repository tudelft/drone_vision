...
/* image_blob_labeling - labels blobs in a UYVY image
 *
 */
...
{
  if (input->type != output->type || input->type != IMAGE_YUV422)
  {
    return;
  }
  ...
  for (i = 0; i < labels_cnt; i++) {
    ...
    labels[i].cgx = labels[i].x_sum / labels[i].pixel_cnt * 2;
    labels[i].cgy = labels[i].y_sum / labels[i].pixel_cnt;
  }
